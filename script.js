...
async function sendMessageWithRetry(group, messageText, imageUrl = null) {
    ...
    const addStatusToList = (status, error = null, apiResponse = null) => {
        ...
        statusItem.innerHTML = `
            ...
            ${apiResponse ? `<div style="color: #1976d2; margin-top: 4px;"><strong>מזהה הודעה:</strong> ${apiResponse.messageId || 'לא התקבל'}</div>` : ''}
        `;
            ...
            if (!apiResponse) {
             ...
             if (!apiResponse.messageId) {
                  sendResults.push({
                       ...
                       chatId: group.id,
                      status: 'warning',
                      error: 'תגובה לא תקינה מהשרת (אין messageId)',
                       fullResponse: apiResponse
                     });
                   addStatusToList('אזהרה', 'תגובה לא תקינה מהשרת (אין messageId)', apiResponse);
                  console.warn(`תגובה לא תקינה מהשרת (אין messageId) for ${group.name}:`, apiResponse);
